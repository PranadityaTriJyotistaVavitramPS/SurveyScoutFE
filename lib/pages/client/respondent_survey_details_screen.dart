...
class RespondentSurveyDetailsScreen extends StatefulWidget {
  @override
  _RespondentSurveyDetailsScreenState createState() => _RespondentSurveyDetailsScreenState();}


class _RespondentSurveyDetailsScreenState extends State<RespondentSurveyDetailsScreen> {
  ...
  void _calculateTotalKomisi() {
    ...
    int formattedComission = int.tryParse(commissionText.replaceAll(RegExp(r'[^0-9]'), '').trim()) ?? 0;
    ...
    int commission = int.tryParse(commissionText.replaceAll(RegExp(r'[^0-9]'), '').trim()) ?? 0;
    int respondentCount = int.tryParse(_projectRespondentAmountController.text) ?? 0;
        ...
        TextField(
          ...
          decoration: InputDecoration(
            ...
            prefixIcon: SizedBox(
              width: 24,
              height: 24,
              child: Center(child: Iconify(Ic.people, color: Color(0xFF826754))),
            ),
            filled: true,
  ...
  Widget _buildKomisiTextField({
    ...
    return Column(
      ...
      children: [
        ...
        TextField(
          controller: controller,
          keyboardType: TextInputType.numberWithOptions(decimal: false), // Number-only keyboard
          inputFormatters: [
            FilteringTextInputFormatter.digitsOnly, // Allow only numbers
          ],
          onChanged: (value) {
            String numericValue = value.replaceAll(RegExp(r'[^0-9]'), ''); // Strip non-numeric characters
            if (numericValue.isNotEmpty) {
              int parsedValue = int.parse(numericValue);
              String formattedValue = "Rp ${NumberFormat("#,###", "id_ID").format(parsedValue)}";
              ...
              _saveProjectCommission(parsedValue.toString()); // Save only number in cache
            }
          ...
          decoration: InputDecoration(
            hintText: "Rp 0",
            hintStyle: GoogleFonts.nunitoSans(color: Color(0xFFA3948D)),
            prefixIcon:  SizedBox(width: 24, height: 24, child: Center(child: Iconify(Ph.money,color: Color(0xFF826754)))), // Display "Rp" permanently
  ...
  void initState() {
    ...
    _projectQualificationController.text = GetStorage().read('project_qualification') ?? '';
    _projectOutcomeController.text = GetStorage().read('project_outcome') ?? '';
    _projectCommissionController.text = GetStorage().read('project_commission') ?? '';
    ...
    DateTime defaultDeadline = DateTime.now().add(Duration(days: 7)).copyWith(hour: 23, minute: 59);

    // Format correctly
    final DateFormat formatter = DateFormat('HH.mm, EEEE, dd MMMM yyyy', 'id_ID');
              ...
              children: [
                Row(
                  children: [
                    ...
                    Expanded(child: _buildRespondentAmountTextField(controller: _projectRespondentAmountController)),
                  ],
                ),
        ...
        TextField(
          ...
          decoration: InputDecoration(
            ...
            hintStyle: TextStyle(
              ...
              fontWeight: FontWeight.w400, // Ensure it's not bold overriding italic
            ),
            prefixIcon:  SizedBox(width: 24, height: 24, child: Center(child: icon)),
            filled: true,
            fillColor: Colors.white,
            border: OutlineInputBorder(
              borderRadius: BorderRadius.circular(12),
